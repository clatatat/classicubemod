...
static void Physics_HandleRedOreDust(int index, BlockID block) {
	...
	if (y > 0) {
		...
		if (Blocks.Draw[below] == DRAW_OPAQUE) return; /* Still has support */
	}
...
static void Physics_PlaceRedOreDust(int index, BlockID block) {
	...
	if (x > 0 && World_GetBlock(x - 1, y, z) == BLOCK_RED_ORE_DUST)
		Game_UpdateBlock(x - 1, y, z, BLOCK_RED_ORE_DUST);
	if (x < World.MaxX && World_GetBlock(x + 1, y, z) == BLOCK_RED_ORE_DUST)
		Game_UpdateBlock(x + 1, y, z, BLOCK_RED_ORE_DUST);
	if (z > 0 && World_GetBlock(x, y, z - 1) == BLOCK_RED_ORE_DUST)
		Game_UpdateBlock(x, y, z - 1, BLOCK_RED_ORE_DUST);
	if (z < World.MaxZ && World_GetBlock(x, y, z + 1) == BLOCK_RED_ORE_DUST)
		Game_UpdateBlock(x, y, z + 1, BLOCK_RED_ORE_DUST);
}
...
static void Physics_DeleteRedOreDust(int index, BlockID block) {
	...
	/* Trigger visual updates on neighboring redstone dust blocks */
	/* This causes their textures to recalculate based on removed connections */
	if (x > 0 && World_GetBlock(x - 1, y, z) == BLOCK_RED_ORE_DUST)
		Game_UpdateBlock(x - 1, y, z, BLOCK_RED_ORE_DUST);
	if (x < World.MaxX && World_GetBlock(x + 1, y, z) == BLOCK_RED_ORE_DUST)
		Game_UpdateBlock(x + 1, y, z, BLOCK_RED_ORE_DUST);
...
void Physics_Init(void) {
	...
	/* Red ore dust breaks when block below is removed */
