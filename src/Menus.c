...
static const char* const GenLevel_ThemeNames[] = { "Normal", "Hell", "Paradise", "Woods", "Desert", "Winter", "Moon" };
#define GENLEVEL_THEME_COUNT 7
...
*-------------------------------------------------MenuDropdownOverlay-----------------------------------------------------*
*#########################################################################################################################*/
#define DROPDOWN_MAX_VISIBLE 10
#define DROPDOWN_MAX_OPTIONS 20
...
static struct MenuDropdownOverlay {
	...
	struct FontDesc titleFont, textFont;
	struct TextWidget title;
	struct ButtonWidget options[DROPDOWN_MAX_OPTIONS];
	struct ScrollbarWidget scrollbar;
	int currentValue;
	int visibleStart;
	int visibleCount;
	int totalCount;
	...
	struct Widget* __widgets[DROPDOWN_MAX_OPTIONS + 2];
} MenuDropdownOverlay;

static void MenuDropdownOverlay_UpdateScrollbar(struct MenuDropdownOverlay* s) {
	if (s->totalCount <= s->visibleCount) return;

	s->scrollbar.topRow      = s->visibleStart;
	s->scrollbar.rowsTotal   = s->totalCount;
	s->scrollbar.rowsVisible = s->visibleCount;
}
...
static void MenuDropdownOverlay_RedrawOptions(struct MenuDropdownOverlay* s) {
	...
	for (i = 0; i < s->visibleCount && i < s->totalCount; i++) {
		optionIdx = s->visibleStart + i;
		if (optionIdx >= s->totalCount) break;

		btn = &s->options[i];
		text = String_FromReadonly(s->optionNames[optionIdx]);
		ButtonWidget_Set(btn, &text, &s->textFont);

		/* Highlight currently selected option */
		if (optionIdx == s->currentValue) {
			btn->color = PackedCol_Make(100, 200, 100, 255);
		} else {
			btn->color = PackedCol_Make(127, 127, 127, 255);
		}
	}
}

static void MenuDropdownOverlay_SetVisibleStart(struct MenuDropdownOverlay* s, int start) {
	s->visibleStart = start;
	if (s->visibleStart < 0) s->visibleStart = 0;
	if (s->visibleStart > s->totalCount - s->visibleCount) {
		s->visibleStart = s->totalCount - s->visibleCount;
	}
	if (s->visibleStart < 0) s->visibleStart = 0;

	MenuDropdownOverlay_RedrawOptions(s);
	MenuDropdownOverlay_UpdateScrollbar(s);
}

static void MenuDropdownOverlay_Close(struct MenuDropdownOverlay* s, cc_bool apply) {
	if (apply) {
		s->setValue(s->currentValue);
		MenuOptionsScreen_Update((struct MenuOptionsScreen*)s->sourceScreen, s->sourceBtn);
	}
	Gui_Remove((struct Screen*)s);
}
...
static void MenuDropdownOverlay_OptionClick(void* screen, void* widget) {
	...
	for (i = 0; i < s->visibleCount; i++) {
		if (&s->options[i] == btn) {
			s->currentValue = s->visibleStart + i;
			MenuDropdownOverlay_Close(s, true);
			return;
		}
	}
}
...
static void MenuDropdownOverlay_Init(void* screen) {
	...
	/* Determine visible count based on mode */
	s->visibleCount = Input_TouchMode ? 7 : 10;
	if (Window_UI.Height < 600) s->visibleCount = 5;
	if (s->visibleCount > s->totalCount) s->visibleCount = s->totalCount;
	if (s->visibleCount <= 0) s->visibleCount = 1; /* Safety check */
	...
	/* Calculate initial scroll position to show current selection */
	s->visibleStart = s->currentValue - (s->visibleCount / 2);
	if (s->visibleStart < 0) s->visibleStart = 0;
	if (s->visibleStart > s->totalCount - s->visibleCount) {
		s->visibleStart = s->totalCount - s->visibleCount;
	}
	...
	/* Add option buttons */
	for (i = 0; i < s->visibleCount; i++) {
		ButtonWidget_Add(s, &s->options[i], Input_TouchMode ? 350 : 300, MenuDropdownOverlay_OptionClick);
	}

	/* Add scrollbar if needed */
	if (s->totalCount > s->visibleCount) {
		ScrollbarWidget_Create(&s->scrollbar, Input_TouchMode ? 25 : 20);
		s->widgets[s->numWidgets++] = (struct Widget*)(&s->scrollbar);
		MenuDropdownOverlay_UpdateScrollbar(s);
	}
...
static int MenuDropdownOverlay_KeyDown(void* screen, int key, struct InputDevice* device) {
	...
	if (key == CCKEY_ESCAPE) {
		MenuDropdownOverlay_Close(s, false);
		return true;
	}
	...
	if (key == CCKEY_UP) {
		if (s->currentValue > 0) {
			s->currentValue--;
			if (s->currentValue < s->visibleStart) {
				MenuDropdownOverlay_SetVisibleStart(s, s->visibleStart - 1);
			} else {
				MenuDropdownOverlay_RedrawOptions(s);
			}
		}
	...
	if (key == CCKEY_DOWN) {
		if (s->currentValue < s->totalCount - 1) {
			s->currentValue++;
			if (s->currentValue >= s->visibleStart + s->visibleCount) {
				MenuDropdownOverlay_SetVisibleStart(s, s->visibleStart + 1);
			} else {
				MenuDropdownOverlay_RedrawOptions(s);
			}
	...
	if (key == CCKEY_PAGEUP) {
		MenuDropdownOverlay_SetVisibleStart(s, s->visibleStart - s->visibleCount);
		return true;
	}

	if (key == CCKEY_PAGEDOWN) {
		MenuDropdownOverlay_SetVisibleStart(s, s->visibleStart + s->visibleCount);
		return true;
	}
...
static int MenuDropdownOverlay_PointerDown(void* screen, int id, int x, int y) {
	...
	if (result < 0 && !s->screenMode) {
		MenuDropdownOverlay_Close(s, false);
		return true;
	}

	return result >= 0 || s->screenMode;
}
...
static int MenuDropdownOverlay_MouseScroll(void* screen, float delta) {
	struct MenuDropdownOverlay* s = (struct MenuDropdownOverlay*)screen;
	int steps = Utils_AccumulateWheelDelta(&s->scrollbar.scrollingAcc, delta);

	if (steps) {
		MenuDropdownOverlay_SetVisibleStart(s, s->visibleStart - steps);
		return true;
	}
	return true;
}
...
static void MenuDropdownOverlay_Layout(void* screen) {
	struct MenuDropdownOverlay* s = (struct MenuDropdownOverlay*)screen;
	int i, startY, buttonHeight, scrollbarX;

	/* Determine layout based on mode */
	buttonHeight = Input_TouchMode ? 40 : 30;
	if (Window_UI.Height < 600) buttonHeight = 25;

	startY = -(s->visibleCount * buttonHeight) / 2;
	scrollbarX = Input_TouchMode ? 190 : 165;

	/* Position title */
	Widget_SetLocation(&s->title, ANCHOR_CENTRE, ANCHOR_CENTRE, 0, startY - 40);

	/* Position option buttons */
	for (i = 0; i < s->visibleCount; i++) {
		Widget_SetLocation(&s->options[i], ANCHOR_CENTRE, ANCHOR_CENTRE, 0, startY + i * buttonHeight);
	}

	/* Position scrollbar */
	if (s->totalCount > s->visibleCount) {
		int scrollHeight = s->visibleCount * buttonHeight;
		Widget_SetLocation(&s->scrollbar, ANCHOR_CENTRE, ANCHOR_CENTRE, scrollbarX, startY);
		s->scrollbar.height = scrollHeight;
	}
...
static void MenuDropdownOverlay_ContextRecreated(void* screen) {
	...
	/* Set title */
	title = String_FromReadonly(s->optionName);
	TextWidget_Set(&s->title, &title, &s->titleFont);

	/* Redraw option buttons */
	MenuDropdownOverlay_RedrawOptions(s);
}

static void MenuDropdownOverlay_Free(void* screen) {
	/* No special cleanup needed */
}

static const struct ScreenVTABLE MenuDropdownOverlay_VTABLE = {
	MenuDropdownOverlay_Init,        Screen_NullUpdate,         MenuDropdownOverlay_Free,
...
void MenuDropdownOverlay_Show(const char* optionName, const char* const* names, int count,
	...
	struct MenuDropdownOverlay* s = &MenuDropdownOverlay;
