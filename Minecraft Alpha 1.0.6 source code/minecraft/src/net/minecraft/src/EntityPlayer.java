...
public class EntityPlayer extends EntityLiving {
	...
	public void onLivingUpdate() {
		if(this.worldObj.difficultySetting == 0 && this.health < 20 && this.aJ % 20 * 4 == 0) {
			this.heal(1);
		}
	...
	public void dropPlayerItemWithRandomChoice(ItemStack var1, boolean var2) {
		if(var1 != null) {
			...
			float var5;
			if(var2) {
				var5 = this.aI.nextFloat() * 0.5F;
				float var6 = this.aI.nextFloat() * (float)Math.PI * 2.0F;
				...
				var4 = 0.02F;
				var5 = this.aI.nextFloat() * (float)Math.PI * 2.0F;
				var4 *= this.aI.nextFloat();
				var3.motionX += Math.cos((double)var5) * (double)var4;
				var3.motionY += (double)((this.aI.nextFloat() - this.aI.nextFloat()) * 0.1F);
	...
	public boolean attackEntityFrom(Entity var1, int var2) {
		...
		if(this.health <= 0) {
			return false;
		} else if((float)this.aO > (float)this.heartsHalvesLife / 2.0F) {
			return false;
		} else {
